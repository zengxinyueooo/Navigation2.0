...
public class CommonConfig {
    ...
    @Autowired
    //把这个EmbeddingModel对象交给EmbeddingStoreIngestor和EmbeddingStoreContentRetriever即可，
    // 一个是存储的时候使用，一个是检索的时候使用。
    private EmbeddingModel embeddingModel;

    @Autowired
    private RedisEmbeddingStore redisEmbeddingStore;//把向量化后的数据存储到外部的向量数据库中，并支持向量检索
...
/**
    ...
    public EmbeddingStore store(){//embeddingStore的对象, 这个对象的名字不能重复,所以这里使用store
        ...
        EmbeddingStoreIngestor ingestor = EmbeddingStoreIngestor.builder()
                //.embeddingStore(store)//要把向量化的数据存储到哪
        ...
        //return store;
        return redisEmbeddingStore;  // 返回 Redis 向量存储实例，供其他组件使用
    }
...
/**
    ...
    public ContentRetriever contentRetriever(EmbeddingStore store){
        return EmbeddingStoreContentRetriever.builder()
                ...
                .embeddingStore(redisEmbeddingStore)//设置向量数据库操作对象，指定从 Redis 向量库检索
